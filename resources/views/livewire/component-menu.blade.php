<div class="">
    ...
    <div class="ml-1 mr-1 dark:bg-slate-300 p-4 rounded-lg shadow-md mb-6">
        ...
        <div class="hidden md:flex space-x-4">
            <!-- Botón de Eventos -->
            <button 
                wire:click="setActiveComponent('events')" 
                class="py-2 px-4 rounded-md text-white 
                    {{ $activeButton === 'events' ? 'bg-blue-800' : 'bg-blue-600 hover:bg-blue-700' }} 
            ...
            <!-- Enlaces de Más Acciones -->
            <a href="{{ route('blogs.accions') }}" 
        ...
        <div class="md:hidden">
            ...
            <div id="dropdownMenu" class="{{ $menuOpen ? 'mt-2 space-y-2' : 'hidden' }}">
                ...
                <a href="{{ route('blogs.accions') }}" 
                    class="py-2 px-4 rounded-md text-white bg-blue-600 hover:bg-blue-700 transition duration-200 flex items-center space-x-2">
                    <i class="fas fa-cogs h-5 w-5"></i>
                    <span>Acciones de tus publicaciones</span>
                </a>
    
                <a href="{{ route('blog.create') }}" 
                    class="py-2 px-4 rounded-md text-white bg-blue-600 hover:bg-blue-700 transition duration-200 flex items-center space-x-2">
                    <i class="fas fa-plus-circle h-5 w-5"></i>
                    <span>Crear Post o publicacion</span>
                </a>
            </div>
        </div>
    </div>
    ...
    <div class="p-2 rounded-lg shadow-md">
        @if ($activeButton === 'awards')
            <livewire:doctor-awards />
        @elseif ($activeButton === 'certificates')
            <livewire:upload-certificate />
        @elseif ($activeButton === 'events')
            <livewire:doctor-events />
        @endif
    </div>
</div>
