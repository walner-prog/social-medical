...
public function setRole(Request $request, $userId)
{
    ...
    // Si el rol es 'patient', crear un registro de paciente
    if ($request->role === 'patient') {
        Patient::create([
            'user_id' => $user->id,
            'name' => $user->name,  // Usamos el nombre del usuario
            'email' => $user->email // Usamos el correo del usuario
        ]);
    }
    ...
    if ($request->role === 'doctor') {
        return redirect()->route('doctor.profile', ['user' => $user->id]);
    }
...
{
    return view('auth.doctor-profile', compact('user'));
}
